...
namespace BillingBackend.Services
{
    public class CloudflareR2StorageService : IStorageService
    {
        ...
        public CloudflareR2StorageService(IConfiguration configuration, ILogger<CloudflareR2StorageService> logger)
        {
            ...
            _logger.LogInformation("Initializing CloudflareR2StorageService. AccountId={AccountId}, BucketName={BucketName}, PublicUrl={PublicUrl}", 
                accountId, _bucketName, _publicUrl);
        ...
        {
            _logger.LogInformation("Cloudflare R2 Upload Started: Bucket={Bucket}, Key={Key}, ContentType={ContentType}, Size={Size} bytes", 
                _bucketName, fileName, contentType, fileStream.Length);
            ...
            try
            {
                ...
                _logger.LogInformation("Cloudflare R2 PutObjectAsync completed. HTTP Status Code: {StatusCode}", response.HttpStatusCode);

                var baseUrl = _publicUrl.TrimEnd('/');
                var fileUrl = $"{baseUrl}/{fileName}";
                
                _logger.LogInformation("Generated Cloudflare R2 file URL: {Url}", fileUrl);
                return fileUrl;
            }
